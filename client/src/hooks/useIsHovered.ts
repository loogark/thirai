...
export const useIsHovered = (refs: Array<HTMLElement | null>): boolean[] => {
  ...
  const [isHoveredByRefs, setIsHoveredByRefs] = useState(
  ...
  useLayoutEffect(() => {
    const mouseEnterListeners = new Map(
      refs.map((ref) => [
        ...
        () => {
          if (ref === null) return;
          setIsHoveredByRefs(
            new Map<HTMLElement, boolean>(isHoveredByRefs.set(ref, true))
          );
        },
      ])
    );
    const mouseLeaveListeners = new Map(
      refs.map((ref) => [
        ...
        () => {
          if (ref === null) return;
          setIsHoveredByRefs(
            new Map<HTMLElement, boolean>(isHoveredByRefs.set(ref, false))
          );
        },
      ])
    );
    ...
    for (const i in refs) {
      ...
      if (isHovered !== isHoveredByRefs.get(ref)) {
        setIsHoveredByRefs(
          new Map<HTMLElement, boolean>(isHoveredByRefs.set(ref, isHovered))
        );
      }
    }
    return () => {
